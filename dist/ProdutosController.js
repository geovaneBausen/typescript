...
exports.inicializarProdutos = inicializarProdutos;
    ...
    try {
        ...
        if (produtosJSON) {
            ...
            produtosArray.forEach((p) => {
                const produto = new Produto_1.Produto(p.id, p.nome, p.preco, p.estoque);
...
function adicionarOuAtualizarProduto(event) {
    ...
    if (editIndexField && editIndexField.value) {
        ...
        const novoProduto = new Produto_1.Produto(id, nome, preco, estoque);
        ...
        const produtos = JSON.parse(localStorage.getItem('produtos') || '[]');
        const idExistente = produtos.some((p) => p.id === id);
        if (idExistente) {
...
function adicionarEventosBotoesProduto() {
...
function prepararEdicaoProduto(index) {
    const produtos = JSON.parse(localStorage.getItem('produtos') || '[]');
    const produto = produtos[index];
    // Preenche o formulário com os dados do produto
    document.getElementById('produto-id').value = produto.id.toString();
    document.getElementById('produto-nome').value = produto.nome;
    document.getElementById('produto-preco').value = produto.preco.toString();
    document.getElementById('produto-estoque').value = produto.estoque.toString();
    // Modifica o formulário para modo de edição
    const form = document.getElementById('form-produto');
    // Cria um campo oculto para guardar o índice sendo editado
    let editIndexField = document.getElementById('edit-produto-index');
    if (!editIndexField) {
    ...
    document.getElementById('produto-id').disabled = true;
}
